...
double *PRNG_uniform(int num_of_pts, int s, int r) {
    ...
    if ((uni_rand = (unsigned int *) malloc(num_of_pts*sizeof(unsigned int))) == NULL) {
        fprintf(stderr,"malloc failed\n");
        exit(1);
    ...
    for (i=r; i<num_of_pts; i++) {
        uni_rand[i] = (uni_rand[i-r] + uni_rand[i-s]) % (RAND_MAX);
    } // Calculate the rest of the sequence using the LFG algorithm

    for (i=0; i<num_of_pts; i++) {
        random_num [i] = uni_rand[i] / (double)(RAND_MAX); //Convert the generated integer sequence to U(0,1)
    }
    free(uni_rand);
    return random_num;
}
